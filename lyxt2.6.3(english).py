...
def load_users():
    """Loading User Information from a File."""
    if os.path.exists(users_file):
        with open(users_file, 'r') as f:
            users = json.load(f)
            # Add the "is_banned" and "ban_end_time" fields if these fields do not exist.
...
def register(users):
    """User registration, check whether the username already exists.
...
")
        ...
        save_users(users)
        print("Registration successful! Your UID is:

", uid)
        return uid

def login(users):
    """Login, support login via UID or username.

"""
    while True:
        login_type = input("Please select the login method (1. UID; 2. Username):
            ...
            if uid in users:
                if users[uid]['is_banned']:
                    if users[uid]['ban_end_time'] is None or users[uid]['ban_end_time'] <= datetime.datetime.now():
                        ...
                        save_users(users)
                        print("The account ban has ended and the user has been unblocked!

")
                    else:
                        print("Login failed. The user has been banned!
...
")
                if uid in users and users[uid]['password'] == password:
                    ...
                    print("Login failed. The UID or password is incorrect!
